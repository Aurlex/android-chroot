...
fn install(
	...
	write(root_path.join("etc/resolv.conf"), read_to_string("/etc/resolv.conf")?)?;
	write(root_path.join("etc/hosts"), read_to_string("/etc/hosts")?)?;
	write(root_path.join("etc/hostname.conf"), gethostname().as_encoded_bytes())?;
